...
affine = false
α = 0.1
N = CouplingLayerAffine(nc, nc_hidden; logdet=logdet, activation=SigmoidNewLayer(T(α)), init_id=init_id, affine=affine, T=T)
...
# Forward (CPU vs GPU)
N = CouplingLayerAffine(nc, nc_hidden; logdet=logdet, activation=SigmoidNewLayer(Float32(α)), init_id=init_id, T=Float32)
