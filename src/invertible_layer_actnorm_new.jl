...
function ActNormNew(; logdet::Bool=true, init_id::Bool=false)
    s = Parameter(nothing)
    b = Parameter(nothing)
    AN = ActNormNew(s, b, false, init_id, logdet, false)
    return AN
end
...
function init_fw!(X::AbstractArray{T,N}, AN::ActNormNew) where {T,N}
    if ~AN.init_id
        ...
        μ = similar(X, size(X, N-1)); fill!(μ, 0)
        σ = similar(X, size(X, N-1)); fill!(σ, 1)
    end
    AN.s.data = 1 ./σ
    AN.b.data = -μ./σ
    AN.is_init = true
...
function custom_backward(ΔY::AbstractArray{T,N}, X::AbstractArray{T,N}, AN::ActNormNew; set_grad::Bool=true) where {T,N}
